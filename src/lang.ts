...
export const LANG_OPTIONS = Object.entries(TRANSLATIONS).map(
  ([code, messages]) => ({
...
export const getReturnContext = (userId: number) => returnContexts.get(userId);
...
export const showLanguageSelection = async (
  ...
  const msg = LANG_OPTIONS.map((lang) =>
    isFirst
