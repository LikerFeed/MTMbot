...
const commandRoutes: Record<string, (ctx: Context) => Promise<void>> = {
  start: async ctx => {
    const userId = getUserId(ctx);
    if (userId) clearReturnContext(userId);
    await showLanguageSelection(ctx, undefined, true);
  ...
  lang: async ctx => {
    const userId = getUserId(ctx);
    if (userId) clearReturnContext(userId);
    await showLanguageSelection(ctx, showMainMenu);
  },
};

Object.entries(commandRoutes).forEach(([cmd, handler]) => {
  bot.command(cmd, async ctx =>
    withUser(ctx, async userId => {
      if (cmd === 'start') return handler(ctx);
      const returnTo = getReturnContext(userId);
      if (returnTo) {
        await handler(ctx);
      } else {
