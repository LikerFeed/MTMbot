const uk = {
  firstTimeStartMessage: "🇺🇦 Привіт! Будь ласка, оберіть мову",
  languageChoicePrompt: "🇺🇦 Будь ласка, оберіть мову",
  languageButton: "🇺🇦 Українська",
  confirmLanguage: "Мову обрано 🇺🇦 Українська",
  chooseAuth: "Будь ласка, оберіть метод аутентифікації",
  signIn: "🔐 Вхід",
  signUp: "🔏 Реєстрація",
  mainMenuMessage:
    "🏠 Ласкаво просимо до головного меню!\nБудь ласка, оберіть опцію",
  logout: "🚪 Вихід",
  successLogout: "🚪 Ви успішно вийшли",
  tasks: "📋 Задачі",
  noTasks: "📋 У вас ще немає задач",
  clickToCreateTask: "➕ Натисніть кнопку, щоб створити задачу",
  tasksMenu: "📋 Ваші задачі",
  categories: "🗂️ Категорії",
  categoriesMenu: "🗂️ Ваші категорії",
  categoryFetchFail: "❌ Не вдалося отримати категорії",
  categoryNotFound: "❌ Категорія не знайдена",
  categoryTitle: "📝 Назва категорії",
  editCategory: "✏️ Редагувати категорію",
  deleteCategory: "🗑️ Видалити категорію",
  showLinkedTask: "🏷️ Привʼязані задачі",
  currentCategoryTitle: "📝 Поточна назва категорії",
  enterNewCategoryTitle: "🔤 Введіть нову назву категорії:",
  categoryTitleUpdateFail: "❌ Не вдалося оновити назву категорії!",
  categoryTitleUpdateSuccess: "✅ Назва категорії оновлена!",
  enterCategoryTitle: "📝 Введіть назву категорії:",
  categoryCreatedSuccess: "✅ Категорію успішно створено!",
  categoryCreatedFail: "❌ Не вдалося створити категорію!",
  areYouSureDeleteCategory: "🗑️ Ви впевнені, що хочете видалити цю категорію?\nЦю дію не можна скасувати!",
  yesDeleteCategory: "✅ Видалити категорію",
  noDeleteCategory: "❌ Не видаляти категорію",
  categoryDeleteSuccess: "✅ Ваша категорія успішно видалена!",
  categoryDeleteCancel: "❌ Видалення категорії скасовано!",
  categoryDeleteFail: "⚠️ Не вдалося видалити категорію",
  profile: "👤 Профіль",
  profileMenu: "👤 Ваш профіль",
  editProfile: "✏️ Редагувати профіль",
  editProfileMenu: "✏️ Редагуйте свій профіль",
  editUsername: "👤 Редагувати ім'я",
  editPassword: "🔒 Редагувати пароль",
  enterOldPassword: "🔒 Введіть ваш старий пароль:",
  enterNewPassword: "🔒 Введіть ваш новий пароль:",
  passwordUpdateFail: "❌ Не вдалося оновити пароль!",
  passwordUpdateSuccess: "✅ Пароль оновлено!",
  backToProfile: "🔙 До профілю",
  currentUsername: "👤 Поточне ім'я користувача",
  enterNewUsername: "🔤 Введіть нове ім'я користувача:",
  usernameUpdateFail: "❌ Не вдалося оновити ім'я користувача!",
  usernameUpdateSuccess: "✅ Ім'я користувача оновлено!",
  deleteProfile: "🗑️ Видалити профіль",
  areYouSureDeleteProfile: "🗑️ Ви впевнені, що хочете видалити свій профіль?\nЦю дію не можна скасувати!",
  yesDeleteProfile: "✅ Видалити профіль",
  noDeleteProfile: "❌ Не видаляти профіль",
  deleteProfileSuccess: "✅ Ваш профіль успішно видалено!",
  deleteProfileCancel: "❌ Видалення профілю скасовано!",
  deleteProfileFail: "⚠️ Не вдалося видалити профіль",
  faq: "❓ ЧаПи",
  faqMenu: "❓ Часто задавані питання",
  backToMainMenu: "🏠 До головного меню",
  createTask: "➕ Створити задачу",
  createCategory: "➕ Створити категорію",
  backToTasks: "📋 До задач",
  backToCategories: "🗂️ До категорій",
  question1: "Що таке MyTaskManager?",
  question2: "Як створити категорію?",
  question3: "Як створити задачу?",
  question4: "Як редагувати задачу та/чи категорію?",
  question5: "Як видалити задачу та/чи категорію?",
  question6: "Як додати посилання до задачі?",
  question7: "Як поділитися задачею з іншими користувачами?",
  question8: "Які фільтри для категорій існують?",
  question9: "Як розвиватиметься проект у майбутньому?",
  question10: "Які моральні цінності розробницької команди?",
  answer1:
    "Це програма, яка дозволить вам створювати різні нотатки, завдання, прикріплювати посилання, файли, відео тощо. Ви можете також спостерігати за своєю продуктивністю у спеціально зробленій для цього вкладці профілю. Є можливість поділитися завданням з другом. а також ми в процесі додавання функціоналу системи управління проектами як у Джирі",
  answer2:
    "Натиснути кнопку створити категорію в сайдбарі або модальному вікні для редагування завдання натиснути аналогічну кнопку",
  answer3:
    "Натиснути кнопку створити завдання праворуч угорі на головній сторінці",
  answer4:
    "Натисніть на іконку олівця праворуч від категорії чи завдання та побачите модальне вікно",
  answer5:
    "Натисніть на іконку сміття праворуч від категорії чи завдання та побачите модальне вікно",
  answer6:
    "Натисніть на іконку плюсика праворуч від категорії або завдання та побачите модальне вікно, де введіть посилання та натисніть підтвердити",
  answer7:
    "Натисніть на іконку стрілочки праворуч від категорії або завдання та побачите модальне вікно, де введіть айді користувача, яку він вам дасть та натисніть підтвердити",
  answer8:
    "На даний момент є сортування за дедлайном та статусом виконання, надалі додамо можливість знайти ті завдання, якими з вами поділилися",
  answer9:
    "Ми додамо можливість створювати завдання із підпунктами, ділитися з іншими користувачами завданнями так. щоб вони підтверджували прийняття чи відхиляли. Додамо повідомлення та можливість листування з користувачами. Систему управління проектом canban, додамо підтвердження реєстрації та відновлення паролю поштою, буде можливість прикріплювати до завдань відео, аудіо, файли та зображення",
  answer10:
    "Робити якісно і швидко, при цьому кожен з нас має роботу чи університет, тому ми іноді працюємо не так швидко, як хотілося б, але ви працюємо =)",
  chooseQuestion: "❓ Оберіть питання, щоб отримати відповідь",
  chooseAnotherQuestion: "👉 Оберіть інше питання або поверніться в меню",
  showQuestions: "📋 Показати питання ще раз",
  enterEmail: "📧 Будь ласка, введіть вашу електронну пошту:",
  invalidEmail: "❌ Неправильний формат електронної пошти\nБудь ласка, спробуйте ще раз:",
  enterPassword: "🔒 Будь ласка, введіть ваш пароль:",
  invalidPassword: "❌ Неправильний формат пароля\nБудь ласка, спробуйте ще раз:",
  successSignIn: "🔓 Ви успішно увійшли!",
  enterUsername: "👤 Будь ласка, введіть ваше ім'я користувача:",
  invalidUsername: "❌ Неправильний формат імені користувача\nБудь ласка, спробуйте ще раз:",
  repeatPassword: "🔁 Повторіть ваш пароль:",
  passwordMismatch: "❌ Паролі не збігаються\nБудь ласка, спробуйте ще раз:",
  successSignUp: "🔓 Ви успішно зареєструвалися!",
  signInFailed: "❌ Не вдалося увійти",
  signUpFailed: "❌ Не вдалося зареєструватися",
  loginError: "Помилка входу",
  registerError: "Помилка реєстрації",
  fetchError: "Помилка отримання даних",
  alreadyAuthorized: "🔓 Ви вже авторизовані!",
  prev: "⬅️ Назад",
  next: "Далі ➡️",
  taskFetchFailed: "❌ Не вдалося отримати задачі",
  taskTitle: "📝 Назва задачі",
  taskDescription: "💬 Опис задачі",
  taskNotFound: "❌ Задача не знайдена",
  completed: "✅ Виконано",
  notCompleted: "❌ Не виконано",
  noDeadline: "❌ Немає дедлайну",
  noCategories: "🗂️ У вас ще немає категорій",
  noLinks: "❌ Немає посилань",
  taskStatus: "📌 Статус задачі",
  taskDeadline: "📅 Дедлайн задачі",
  taskCategories: "🏷️ Категорії задачі",
  taskLinks: "🔗 Посилання задачі",
  deleteTask: "🗑️ Видалити задачу",
  areYouSureDeleteTask: "🗑️ Ви впевнені, що хочете видалити цю задачу?\nЦю дію не можна скасувати!",
  yesDeleteTask: "✅ Видалити задачу",
  noDeleteTask: "❌ Не видаляти задачу",
  deleteTaskSuccess: "✅ Ваша задача успішно видалена!",
  deleteTaskCancel: "❌ Видалення задачі скасовано!",
  deleteTaskFail: "⚠️ Не вдалося видалити задачу",
  editTask: "📝 Редагувати задачу",
  editTaskTitle: "📝 Назва задачі",
  editTaskDescription: "💬 Опис задачі",
  editTaskDeadline: "📅 Дедлайн задачі",
  editTaskCategories: "🏷️ Категорії задачі",
  editTaskLinks: "🔗 Посилання задачі",
  editTaskStatus: "📌 Статус задачі",
  backToTask: "🔙 До задачі",
  currentTaskTitle: "📝 Поточна назва задачі",
  enterNewTaskTitle: "🔤 Введіть нову назву задачі:",
  taskTitleUpdateFail: "❌ Не вдалося оновити назву задачі!",
  taskTitleUpdateSuccess: "✅ Назва задачі оновлена!",
  currentTaskDescription: "💬 Поточний опис задачі",
  enterNewTaskDescription: "🔤 Введіть новий опис задачі:",
  taskDescriptionUpdateFail: "❌ Не вдалося оновити опис задачі!",
  taskDescriptionUpdateSuccess: "✅ Опис задачі оновлено!",
  taskStatusUpdateFail: "⚠️ Не вдалося оновити статус задачі!",
  taskMarkedCompleted: "✅ Задача позначена як виконана!",
  taskMarkedUncompleted: "❌ Задача позначена як не виконана!",
  currentTaskDeadline: "📅 Поточний дедлайн задачі",
  enterNewTaskDeadlineFormat: "🔤 Введіть новий дедлайн задачі у форматі ДД.ММ.РР (або напишіть «нема» для очищення):",
  invalidTaskDeadlineFormat: "❌ Неправильний формат дедлайну задачі\nБудь ласка, спробуйте ще раз у форматі ДД.ММ.РР або напишіть «нема» для очищення",
  taskDeadlineUpdateFail: "❌ Не вдалося оновити дедлайн задачі!",
  taskDeadlineUpdateSuccess: "✅ Дедлайн задачі оновлено!",
  taskDeadlineCleared: "✅ Дедлайн задачі очищено!",
  currentTaskLinks: "🔗 Поточні посилання задачі",
  enterNewTaskLinks: "🔗 Введіть нові посилання задачі (розділені пробілами):",
  taskLinksUpdateFail: "❌ Не вдалося оновити посилання задачі!",
  taskLinksUpdateSuccess: "✅ Посилання задачі оновлено!",
  taskLinksCleared: "✅ Посилання задачі очищено!",
  enterTaskTitle: "📝 Введіть назву задачі:",
  enterTaskDescription: "💬 Введіть опис задачі:",
  taskCreatedSuccess: "✅ Задача успішно створена!",
  taskCreatedFail: "❌ Не вдалося створити задачу!",
  profileLoadFailed: "❌ Не вдалося завантажити профіль",
  profileInfo: "👤 Інформація про профіль",
  username: "👤 Ім'я користувача",
  email: "📧 Електронна пошта",
  createdAt: "📅 Дата створення",
  categoryLoadError: "❌ Не вдалося завантажити категорії",
  chooseCategoryToggle: "🗂️ Оберіть категорію для перемикання",
  categoryUpdateFail: "❌ Не вдалося оновити категорію",
  noTasksInCategory: "❌ У цій категорії немає задач",
  backToCategory: "🔙 До категорії",
  linkedTasksMenu: "🏷️ Меню прив'язаних задач",
  sortTasks: "📊 Сортувати",
  chooseSortOption: "📊 Оберіть опцію сортування",
  sortByStatus: "📌 За статусом",
  sortByDeadline: "📅 За дедлайном",
  sortByCreatedAt: "📥 За новизною",
  sortByUpdatedAt: "🛠️ За оновленням",
  noTaskCategories: "❌ Немає категорій",
};

export default uk;
